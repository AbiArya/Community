...
interface BatchGenerationResult {
  ...
  matchDistribution?: {
    ...
    usersWithOneMatch: number;
    usersWithTwoMatches: number;
    usersWithThreeOrMoreMatches: number;
  };
  summary: {
    startTime: string;
    endTime: string;
...
): Promise<{ success: boolean; matchCount: number; error?: string; storedMatches?: Array<{ user_1_id: string; user_2_id: string }> }> {
  try {
    ...
    let candidates = await fetchMatchCandidates(
      supabase,
      userId,
    ...
    if (context) {
      const filteredMatches: typeof potentialMatches = [];
...
/**
 * Context for tracking match state during batch generation
 * Uses flexible caps: min=1, target=2, max=3
 */
interface MatchContext {
  /** Map of user_id -> number of matches this week */
  matchCountByUser: Map<string, number>;
  /** Set of existing match pairs (normalized as "smallerId:largerId") */
  existingPairs: Set<string>;
  /** Minimum matches per user (guaranteed coverage) */
  minMatchesPerUser: number;
  /** Target matches per user (ideal) */
  targetMatchesPerUser: number;
  /** Maximum matches per user (hard cap) */
 ...
 */
function normalizePairKey(userId1: string, userId2: string): string {
...
async function buildMatchContext(
  supabase: SupabaseClientType,
  currentWeek: string,
  options: { min: number; target: number; max: number } = { 
...
function updateMatchContext(
  context: MatchContext,
  newMatches: Array<{ user_1_id: string; user_2_id: string }>
): void {
...
/**
 * User with their eligible candidate count (for priority sorting)
 */
interface UserWithCandidateCount {
  userId: string;
 ...
 */
function countEligibleCandidates(
  userId: string,
  allUserIds: string[],
  context: MatchContext
): number {
  ...
  for (const candidateId of allUserIds) {
    ...
    if (context.existingPairs.has(pairKey)) continue;
    
    // Check if candidate is at max
    const candidateMatchCount = context.matchCountByUser.get(candidateId) || 0;
    if (candidateMatchCount >= context.maxMatchesPerUser) continue;
 ...
 */
function sortUsersByPriority(
  userIds: string[],
  context: MatchContext,
  targetMatches: number
): UserWithCandidateCount[] {
  const usersWithCounts: UserWithCandidateCount[] = userIds.map(userId => ({
    userId,
 ...
 */
async function deleteMatchesForWeek(
  supabase: SupabaseClientType,
  week: string
): Promise<number> {
  const { data, error } = await supabase
 ...
 */
async function processUserInPhase(
  supabase: SupabaseClientType,
  userId: string,
  currentWeek: string,
  context: MatchContext,
 ...
 */
async function runBatchGeneration(options: BatchGenerationOptions = {}): Promise<BatchGenerationResult> {
  ...
  try {
    ...
    for (let i = 0; i < phase1Users.length; i++) {
      ...
      const result = await processUserInPhase(
        supabase,
        userId,
        currentWeek,
        matchContext,
    ...
    for (const userId of allUsers) {
      ...
      if (count === 0) usersWithZero++;
      else if (count === 1) usersWithOne++;
      else if (count === 2) usersWithTwo++;
      else if (count >= 3) usersWithThree++;
    }
